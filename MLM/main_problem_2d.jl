...
Layer_hidden = Flux.Dense(2=>10, sigmoid)
Layer_output = Flux.Dense(10=>1)
model_2d = Flux.Chain(Layer_hidden, Layer_output)
x1_model = Float32.(collect(LinRange(0,1,41)))
x2_model = x1_model
y_model_2d = Float32.(cos.(20*(x1_model+x2_model)))
data_model_2d = [([x1_model[i],x2_model[i]],[y_model_2d[i]]) for i in 1:length(x1_model)]
loss_model(model,x,y) = norm(model(x).-y)
learning_rate = 0.01
opt_state_2d = Flux.setup(Flux.Adam(learning_rate),model_2d)
Flux.train!(loss_model,model_2d,data_model_2d,opt_state_2d)
input_weight_2d = Layer_hidden.weight
input_bias_2d = Layer_hidden.bias
output_weight_2d = vec(Layer_output.weight)
output_bias_2d = Layer_output.bias
...
function MLM_2D(input_weights,input_biases,output_weights,output_bias,x1,x2,σ,l)
    ...
    while norm(grad_obj_2d(input_weights,input_biases,output_weights,output_bias,x1,x2,σ),2) > ϵ
       ...
       g_w2 = obj_2d_approx_w1(input_weights,input_biases,output_weights,output_bias,x1,x2,σ)
       ...
       R_grad = R_block*vcat(input_weights[:,1],input_weights[:,2],input_biases,output_weights)
       ...
       if l>1 && norm(R_grad,2) >= κ_H*norm(g_withoutd,2) && norm(R_grad) > ϵ_H 
        ...
        s_h[1:end-1] .= P_block*(s_H[1:end-1].-vcat(reshape(w_H',:,1),b_H,v_H))
        s_h[end] = s_H[end]
...
MLM_2D(input_weight_2d,input_bias_2d,output_weight_2d,output_bias_2d,x1_model,x2_model,sigmoid,2)
